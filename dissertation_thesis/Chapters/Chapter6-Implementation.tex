...
\emph{Figure} \ref{fig:ONNXBeginning} presents the ONNX representation of a network architecture called \emph{TFC} and corresponding to three fully-connected layers put one after each other. This ONNX representation is obtained through the export modules in Brevitas, and particularly \texttt{brevitas.onnx}. This module is still under active development by the Xilinx team. The visualisation is made possible through the \emph{Netron} tool.

% TODO CITE GITHUB BREVITAS FINN ONNX NETRON NN_BENCHMARK

% TFC ONNX THROUGH NETRON
\begin{figure}[htbp]
	\centering
		\includegraphics[width=12cm]{Figures/ONNXBeginning.png}
	\caption[TFC ONNX representation]{TFC ONNX Representation}
